	...
	if (ON == 0) {
		...
		for (i = 0; i < 5; i++) {
			...
			if (!((unchar)__i2c_read_byte(TX_P2, TX_PLL_FILTER5)
				...
				pr_info("%s %s : VBUS power can not be \
					 supplied\n", LOG_TAG, __func__);
		}
	}
}
	...
	if (g_edid_break == 0) {
		...
		for (i = 0; i < 16; i++) {
			if (org_buf[0x70 + i] != temp_buf[i]) {
				pr_info("%s %s : different checksum and \
					 blocks num\n", LOG_TAG, __func__);
				return_flag = 1;
				break;
			}
...
static void hpd_irq_process(void)
{
	...
	if (data_buf[1] & CP_IRQ) {
		if (HDCP_state > HDCP_WAITTING_FINISH
			...
			if (c1 & 0x04) {
				system_state_change_with_case(STATE_HDCP_AUTH);
				pr_info("%s %s : IRQ:____________HDCP \
					 Sync lost!\n", LOG_TAG, __func__);
			}
		}
	}
	...
	if (HDCP_state > HDCP_HW_ENABLE
		...
		if (bytebuf[0] & SP_TX_HDCP_AUTH_PASS) {
			...
			if ((bytebuf[1] & 0x08) || (bytebuf[0] & 0x80)) {
				...
				pr_info("%s %s : Authentication pass in \
					 Auth_Done\n", LOG_TAG, __func__);
...
static void sp_tx_lt_done_int_handler(void)
{
	...
	if (sp_tx_LT_state == LT_WAITTING_FINISH
		...
		if (c & 0x70) {
			c = (c & 0x70) >> 4;
			pr_info("%s %s : LT failed in interrupt, ERR code = \
					 %.2x\n", LOG_TAG, __func__, (uint) c);
			sp_tx_LT_state = LT_ERROR;
		} else {
...
static void hdmi_rx_avmute_int(void)
{
	...
	sp_read_reg(RX_P0, HDMI_STATUS, &avmute_status);
...
static void hdmi_rx_new_vsi_int(void)
{
	...
	if ((hdmi_video_format & 0xe0) == 0x40) {
		pr_info("%s %s : 3D VSI packet is detected. \
				 Config VSC packet\n", LOG_TAG, __func__);
...
static void sp_tx_hpd_int_handler(unchar hpd_source)
{
	...
	case HPD_CHANGE:
		...
		if (__i2c_read_byte(TX_P0, SP_TX_SYS_CTRL3_REG) & HPD_STATUS) {
			...
			if (__i2c_read_byte(TX_P0, SP_TX_SYS_CTRL3_REG)
				& HPD_STATUS) {
				hdmi_rx_set_hpd(0);
				sp_tx_set_sys_state(STATE_WAITTING_CABLE_PLUG);
			}
...
void hdmi_rx_show_video_info(void)
{
	...
	if ((c & 0x30) == 0x00)
		...
		pr_info("%s %s : Input Pixel Clock = 2x Video Clock. \
				 Repeated.\n", LOG_TAG, __func__);
	else if ((c & 0x30) == 0x30)
		pr_info("%s %s : Input Pixel Clock = 4x Vvideo Clock. \
				 Repeated.\n", LOG_TAG, __func__);
	...
	if (c & 0x02)
		pr_info("%s %s : Output Video Using Rising Edge \
				 To Latch Data.\n", LOG_TAG, __func__);
	else
		pr_info("%s %s : Output Video Using Falling Edge \
				 To Latch Data.\n", LOG_TAG, __func__);
